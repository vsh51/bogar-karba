...
public class AdminAuthService : IAdminAuthService
{
    ...
    public async Task<AdminLoginResult> LoginAsync(string userName, string password)
    {
        ...
        if (user is null)
        {
            ...
            return new AdminLoginResult { Succeeded = false, ErrorMessage = "Invalid username or password." };
        }
        ...
        var passwordValid = await _repository.CheckPasswordAsync(user, password);
        if (!passwordValid)
        {
        ...
        return new AdminLoginResult { Succeeded = true };
    }
